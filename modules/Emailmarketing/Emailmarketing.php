...
class Emailmarketing extends Vtiger_CRMEntity {
	...
	var $related_tables = Array( 
					'vtiger_emailmarketingcf' => Array('emailmarketingid')
					);
	...
	var $list_fields = Array (
		...
		'Emailmarketing No' => Array('emailmarketing', 'emailmarketingno'),
/*'Emailmarketing No'=> Array('emailmarketing', 'emailmarketingno'),*/
		'Assigned To' => Array('crmentity','smownerid')
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Emailmarketing No' => 'emailmarketingno',
/*'Emailmarketing No'=> 'emailmarketingno',*/
		'Assigned To' => 'assigned_user_id'
	);

	// Make the field link to detail view
	var $list_link_field = 'emailmarketingno';
...
/*'Emailmarketing No'=> 'emailmarketingno',*/
	...
	var $popup_fields = Array ('emailmarketingno');

	// For Alphabetical search
	var $def_basicsearch_col = 'emailmarketingno';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'emailmarketingno';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('emailmarketingno','assigned_user_id');

	var $default_order_by = 'emailmarketingno';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			Emailmarketing::checkWebServiceEntry();
			Emailmarketing::createUserFieldTable($moduleName);
			Emailmarketing::addInTabMenu($moduleName);
	...
	static function checkWebServiceEntry() {
		...
		$sql       =  "SELECT count(id) AS cnt FROM vtiger_ws_entity WHERE name = 'Emailmarketing'";
		...
		if($adb->num_rows($result) > 0)
		{
			...
			if($no == 0)
			{
				...
				$ws_entitySql = "INSERT INTO vtiger_ws_entity ( id, name, handler_path, handler_class, ismodule ) VALUES".
						  " (?, 'Emailmarketing','include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation' , 1)";
