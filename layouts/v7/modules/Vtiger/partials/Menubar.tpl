{*+**********************************************************************************
* The contents of this file are subject to the vtiger CRM Public License Version 1.1
* ("License"); You may not use this file except in compliance with the License
* The Original Code is: vtiger CRM Open Source
* The Initial Developer of the Original Code is vtiger.
* Portions created by vtiger are Copyright (C) vtiger.
* All Rights Reserved.
************************************************************************************}

{if $MENU_STRUCTURE}
{assign var="topMenus" value=$MENU_STRUCTURE->getTop()}
{assign var="moreMenus" value=$MENU_STRUCTURE->getMore()}

<div id="modules-menu" class="modules-menu">
	{foreach key=moduleName item=moduleModel from=$SELECTED_CATEGORY_MENU_LIST}
		{assign var='translatedModuleLabel' value=vtranslate($moduleModel->get('label'),$moduleName )}
		<ul title="{$translatedModuleLabel}" class="module-qtip {$translatedModuleLabel}">
			<li {if $MODULE eq $moduleName}class="active"{else}class=""{/if}>
				<a href="{$moduleModel->getDefaultUrl()}&app={$SELECTED_MENU_CATEGORY}">
					{$moduleModel->getModuleIcon()}
					<span>{$translatedModuleLabel}</span>
				</a>
			</li>
		</ul>
	{/foreach}
        {if $MODULE !=='EmailTemplates' && $MODULE !=='HelpDesk' && $MODULE !=='Support' && $MODULE !=='Invoice' && $MODULE !=='PurchaseOrder' }
	<ul class="module-qtip" data-hasqtip="3" oldtitle="Opportunities" title="My360 Opportunity" aria-describedby="qtip-3">
		<li {if $MODULE eq Potentials}class="active"{else}class=""{/if}>
				<a href="index.php?module=Potentials&view=List&app=MARKETING">
					<i class="vicon-potentials"></i>
					<span>Opportunities</span>
				</a>
			</li>
	</ul>
{/if}
 {if $MODULE ==='HelpDesk' }


<ul class="module-qtip">
<li>									
<a href="index.php?module=MailManager&view=List" title="Mail Manager">
<i class="faicons fa fa-envelope" ></i>
<span>Mail Manager</span>
</a>
</li>

<li>
<a href="index.php?module=Calendar&view=List" title="Activities">
<i class="vicon-calendarlist" ></i>
<span>Activities</span>
</a>
</li>

<li>
<a href="index.php?module=Calendar&view=Calendar" title="Calendar">
<i class="fa fa-calendar" ></i>
<span>Calendar</span>
</a>
</li>
	
{/if}
</div>
{/if}
